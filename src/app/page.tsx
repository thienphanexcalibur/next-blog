    ...
    <div className="grid grid-cols-[repeat(auto-fill,minmax(350px,1fr))] gap-6">
      {meaningfulPost
        .sort((a, b) => {
          const id1 = a[0];
          const id2 = b[0];
          const createdTime1 = a[1]?.block[id1].value?.value?.created_time;
          const createdTime2 = b[1]?.block[id2].value?.value?.created_time;
        ...
        .map(([id, data], index) => {
          ...
          return (
            <NotionPost
              ...
              createdTime={data?.block[id].value?.value?.created_time ?? null}
              href={`/posts/${id}`}
              cover={normalizeNotionCoverPath(
                data?.block[id].value?.value?.format?.page_cover,
              )}
            />
          );
        })}
